...
(define (clause-trivial?02 c)
  (not (= (fourth c) (+ (length (second c)) (length (third c))))))




(define (resolve c1 c2)
  (define (pomoc1 wybieram sprawdzam wynik usuniete)
    (cond ((and (null? sprawdzam) (null? wybieram)) (cons wynik (reverse usuniete)))
          ((null? sprawdzam) (cons (append wynik wybieram) (reverse usuniete)))        
          ((null? wybieram) (cons wynik (reverse usuniete)))            
          ((member (car wybieram) sprawdzam) (pomoc1 (cdr wybieram) (cdr (member (car wybieram) sprawdzam)) wynik (cons (car wybieram) usuniete )))
          (else
           (pomoc1 (cdr wybieram) sprawdzam (append wynik (list (car wybieram))) usuniete))))
                
  (define prawda1 (pomoc1 (second c1) (third c2) null null))
  (define falsz1 (pomoc1 (third c1) (second c2) null null)) 
  
  (define prawda2 (pomoc1 (second c2) (third c1) null null))
  (define falsz2 (pomoc1 (third c2) (second c1) null null))
  (define prawda (remove-duplicates-vars (merge-vars (car prawda1) (car prawda2))))
  (define falsz  (remove-duplicates-vars (merge-vars (car falsz1)  (car falsz2) )))
  (define usuniete (remove-duplicates (merge-vars (merge-vars (cdr prawda1) (cdr prawda2))
                                                  (merge-vars (cdr falsz1 ) (cdr falsz2 )))))
  (if (or (not (= (+ (length prawda1) (length falsz1)) (fourth c1)))
        ...
        (let ((dlugosc (+ (length prawda) (length falsz))))
          (if (= dlugosc 0)
              #f
            (list 'res-int prawda falsz dlugosc (list 'resolve usuniete (fifth c1) (fifth c2)))))
      #f))
...
(define f1 (cnf (clause (literal true 'x) (literal false 'y))
                  (clause (literal false 'x))))
(define f2 (cnf (clause (literal true 'x) (literal false 'x))))

(define claun1 (form->clauses f1))
(define claun2 (form->clauses f2))
(define d1 '(res-int (x) (y) 2 'axiom))
(define d2 '(res-int () (x) 1 'axiom))


(resolve (car claun1) (cadr claun1))
(resolve (cadr claun1) (car claun1))
(clause-trivial? (car claun1))
(clause-trivial? (car claun2))
(clause-trivial?01 (car claun1))
(clause-trivial?01 (car claun2))

(define d3 '(res-int (a b c d) (e f g h) 8 'axiom))
(define d4 '(res-int (e f x z) (c d y) 7 'axiom))
(resolve d3 d4)
(define d5 '(res-int (a b c d) (z) 4 'axiom))
(define d6 '(res-int (x y z) (a b c) 6 'axiom))
(resolve d6 d5)

(define t1 '(res-int (x y z) (a b x) 5 'axiom))
(define t2 '(res-int (x) (x) 1 'axiom))
(clause-trivial? t1)
(clause-trivial? t2)
(define t3 '(res-int (a b c) (d e f) 6 'axiom))
(define t4 '(res-int (a a a) (b b b) 2 'axiom))
(clause-trivial? t3)
(clause-trivial? t4)
