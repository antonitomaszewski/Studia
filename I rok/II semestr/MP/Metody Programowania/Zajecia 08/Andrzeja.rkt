...
(define (cxxr-translate e)
  (define (iter rem)
    (if (eq? (car rem) 'r)
        (cxxr-expr e)
        (if (eq? (car rem) 'a)
            (list 'car (iter (cdr rem)))
            (list 'cdr (iter (cdr rem))))))
  (iter (cdr (split-symbol (cxxr-sym e)))))
...
;; lambda-mul
(define (lambda-mul? t)
  (and (tagged-tuple? 'lambda-mul 3 t)
       (var? (second t))))

(define (lambda-mul-cons v e)
  (list 'lambda-mul v e))

(define (lambda-mul-expr e)
  (third e))

(define (lambda-mul-list e)
  (second e))
...
;; closure-mul
(define (closure-mul-cons x expr env)
  (list 'closure-mul x expr env))

(define (closure-mul? c)
  (and (list? c)
       (= (length c) 4)
       (eq? (car c) 'closure-mul)))

(define (closure-mul-list c)
  (cadr c))

(define (closure-mul-expr c)
  (caddr c))

(define (closure-mul-env c)
  (cadddr c))
  ...
  (cond [(const? e)
        ...
        [(lambda-rec? e)
         (closure-rec-cons (lambda-rec-name e)
                           ...
                           env)]
        [(lambda-mul? e)
         (closure-mul-cons (lambda-mul-list e)
                           (lambda-mul-expr e)
                           env)]
        [(app? e)
         (apply-closure
...
(define (apply-closure c args)
  (cond [(closure? c)
        ...
        [(closure-mul? c)
         (eval-env
          (closure-mul-expr c)
          (add-to-env (closure-mul-list c) args (closure-mul-env c)))]))
                      ;(foldr (lambda (x res)
                      ;         (cons-cons (eval-env x (closure-mul-env c))
                      ;                    res))
                      ;       'null
...
(set! e
  (add-to-env
   ...
   (eval
    '(lambda (la)
       (let (inside (lambda-rec (itlia l acu)
                                (if (null? l)
                                    acu
                                    (itlia (cdr l) (cons (car l) acu)))))
         (inside la null))))
   e))
...
(eval-env (list 'reverse (listify '(1 2 3 4))) e)

(eval-env (list 'map 'inc (listify '(1 2 3 4))) e)

(eval '(cdddddr (cons 1 (cons 2 (cons 3 (cons 4 (cons 5 6))))))) 
      ...
      (add-to-env
       ...
       (eval
        '(lambda-rec (eval e)
                     (if (eq? (car e) 'number)
                         (cdr e)
                         ((car e) (cons (eval (cadr e)) (eval (caddr e))))))) e1))
;;(eval-env '(eval (cons + (cons (cons 'number 1) (cons 'number 2)))) e1)
;;(eval-env '(eval (cons 'number 1)) e1)
(eval-env '(eval (cons 'number 1)) e1)
   ...
   (add-to-env
    ...
    '(lambda-rec (arith-eval e)
                 (if (pair? e)
                     (if (pair? (car e))
                         (if (pair? (cdr e))
                             (cons (arith-eval (car e)) (arith-eval (cdr e)))
                             (cons (arith-eval (car e)) (cdr e)))
                         (if (pair? (cdr e))
                             (cons (car e) (arith-eval (cdr e)))
                             (cons (car e) (cdr e))))
                     e)) q))
(eval-env '(arith-eval (cons '+ (cons 1 (cons 2 null)))) q)
(eval-env '(arith-eval '+) q)
(eval '(pair? '+))
(set! q
      (add-to-env
       'op?
...
(set! q
      (add-to-env
       ...
       '(lambda-rec (oblicz e)
                    (if (pair? e)
                        (if (op? (car e))
                            e ;;(cons (car e) (cons (oblicz (cadr e)) (oblicz (caddr e))))
                            e ;;(car e)
                            )
                        e))) q))
;;(eval-env '(oblicz (cons '+ (cons 1 (cons 2 null)))) q)
;;(eval-env '(oblicz 1) q)
(set! q
      (add-to-env
       'proba
...
(set! kolejny
      (add-to-env
       ...
       kolejny))
(display "PATRZ\n")
(eval-env '(wynik (cons '+ (cons 1 (cons 2 null)))) kolejny)
(eval '(cadr (car (cons '+ (cons 1 (cons 2 null))))))
#|(eval-env '(wynik (cons '-
                    (cons (cons '+ (cons 1 (cons 2 null)))
                          (cons '+ (cons 10 (cons 3 null)))))) kolejny)|#
(eval '(caddr (cons '-
                    (cons (cons '+ (cons 1 (cons 2 null)))
                          (cons '+ (cons 10 (cons 3 null)))))))

;;(eval-env '(wynik (cons '+ (cons 1 (cons '+ (cons 2 (cons 3 null)))))) kolejny)
(set! kolejny
      (add-to-env
       'dodawanie
       (eval
        '(lambda-rec (dodawanie e)
                     (if (pair? e)
                         (if (pair? (cdr e))
                             (if (eq? (car e) '+)
                                 (+ (dodawanie (cadr e)) (dodawanie (cddr e)))
                                 0)
                             (car e))
                         e))) kolejny))
(eval-env '(dodawanie (cons '+ (cons 1 2))) kolejny)
(eval-env '(dodawanie (cons '+ (cons 1 (cons '+ (cons 3 (cons 4 null)))))) kolejny)
(eval-env '(dodawanie (cons '+ (cons (cons '+ (cons 5 (cons 6 null)))
                                      (cons '+ (cons 3 (cons 4 null)))))) kolejny)

(eval-env '(dodawanie (cons '+ (cons 1
                                     (cons '+ (cons (cons '+ (cons 5 (cons 6 null)))
                                           (cons '+ (cons 3 (cons 4 null)))))))) kolejny)
(define DoOddania empty-env)
(set!
 DoOddania
 (add-to-env
  'arith-eval
  ...
  '(lambda-rec (arith-eval e)
               (if (pair? e)
                   (if (pair? (cdr e))
                       (if (eq? '+ (car e))
                           (+ (arith-eval (cadr e)) (arith-eval (cddr e)))
                           (if (eq? '- (car e))
                               (- (arith-eval (cadr e)) (arith-eval (cddr e)))
                               (if (eq? '* (car e))
                                   (* (arith-eval (cadr e)) (arith-eval (cddr e)))
...
(eval-env '(arith-eval (cons '- (cons 1
                                     (cons '+ (cons (cons '+ (cons 5 (cons 6 null)))
                                           (cons '+ (cons 3 (cons 4 null))))))))
          DoOddania)
(eval-env '(arith-eval (cons '* (cons 2 (cons '+ (cons 3 (cons 4 null)))))) DoOddania)
(eval-env '(arith-eval (cons '/ (cons 2 (cons 4 null)))) DoOddania)
