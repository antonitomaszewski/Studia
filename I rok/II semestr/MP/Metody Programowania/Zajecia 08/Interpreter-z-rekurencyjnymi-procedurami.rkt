...
(define (lambda-rec-vars e)
  (cdr (second e)))
;; domkniece zwiazac z nazwa

'(let (append ))
(define (moj-append a b)
  (if (null? a)
      b
      (cons (car a) (moj-append (cdr a) b))))
(moj-append '(1 2 3) '(1 3 5))

(define (moj-map f L)
  (if (null? L)
      null
      (cons (f (car L)) (moj-map f (cdr L)))))
(define przyklad-map
  '(let (map
         (lambda-rec
          (if (null? L)
...
(define (append? l)
  (and (list? l)
       ...
       (list? (cadr l))
       (list? (caddr l))))
(define (append-first l)
  (cadr l))
(define (append-second l)
  (caddr l))
