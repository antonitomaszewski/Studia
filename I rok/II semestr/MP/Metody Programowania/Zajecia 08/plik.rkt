...
(define (lambda? t)
  (and (tagged-tuple? 'lambda 3 t)
       (or
        (and (list? (cadr t)) (andmap symbol? (cadr t)))
         (var? (cadr t)))))
...
(define (eval-env e env)
  (cond [(const? e)
        ...
        [(and (list? e)
              (list? (car e))
              (tagged-tuple? 'lambda 3 (car e))
              (var? (lambda-vars (car e))))
         (eval-env (lambda-expr (car e))
                   (add-to-env (lambda-vars (car e)) (cdr e) env))]
...
(eval '(let (map
         ...
         (map (lambda (x) (* x x)) (cons 1 (cons 2 (cons 3 null))))))
...
(eval '(let (eval
             (lambda-rec (eval w)
                         (if (number? w)
                             w
                             ((car w) (eval (cadr w)) (eval (caddr w))))))
         (eval (+ 1 2))))
(eval '(let (number? (eq? 'number (car e)))
         (number? '(number 1))))
(eval '(eq? (car (cons 'number 1)) 'number))
(eval '(let (number? (lambda-rec (number? e)
                                   (eq? 'number (car e))))
         (number? (cons 'number 1))))

(cons + (cons lewe prawe))
(cons number l12321301)
