...
;;; drzewa binarne
;; 'leaf 'node sa tylko po to aby nadawac im te wartosc i aby mozna sie odwolac czy tak czy nie
;; twierdzenie o indukcji dla listy: albo jest (null? car) albo (pair? ) i (list? cdr)
(define (leaf? x)
  (eq? x 'leaf))  ;;(eq? 'yes 'yes) == #t    'nazwa znacznik 
...
(define (node? x)
  (and (list? x)
       (= (length x) 4)
       (eq? (car x) 'node))) ;;poco sprawdzac dlugosc skoro eq? node' zalatwia nam sprawe
...
(define (bst-find x t)
  (cond [(leaf? t)          false] ;;leaf jest po prostu oznaczeniem konca
        [(= x (node-val t)) true]
...
;;; kodowanie Huffmana == laczenie mapowania po wielu listach

(define (concat-map f cs)
  (apply append (map f cs)))
;;(concat-map sqr sqr) (apply proc v ... lst #kw-arg...) -> any
;; procedure? ; any/c list? any/c
;;procedure musi obslugiwac tyle ile values + length(list) 
(apply / 1 2 3 '(5 5))
(define a (compose sqr sqr sqr))
(a 2)
;; compose dziala od konca do poczatku i zwraca wartosc jesli (liczba argumentow p2 ) == (liczba wartosci p1)
;; compose1 chyba potrzebuje przejscia 1-1 z argumentu w wartosc!!! ale nie jestem pewien
...
;; heap == sterta
;;chcemy fragment wlozyc na sterte aby odpowiadala jej wartosc wielkosci
;; to troche jakby wstawiac ksiazke na polke i isc od A...Z po kolei
...
(define (bst-sort list)
  (define (stworz list drzewo)
    (if (null? list)
        drzewo
        (stworz (cdr list) (bst-insert (car list) drzewo))))
  (tree-flatten (stworz list leaf)))

(bst-sort '(5 4 3 221 1 0))
(define (stworz list drzewo)
    (if (null? list)
        drzewo
        (stworz (cdr list) (bst-insert (car list) drzewo))))
(define (delete element t)
  (define (stworz list drzewo)
    (if (null? list)
        drzewo
        (stworz (cdr list) (bst-insert (car list) drzewo))))
  (cond [(= (node-val t) element) (stworz (append (tree-flatten (node-left t))
                                                    (tree-flatten (node-right t)))
                                            leaf)]
        [(< (node-val t) element) (make-node (node-val t) (node-left t) (delete element (node-right t)))]
        [(> (node-val t) element) (make-node (node-val t) (delete element (node-left t)) (node-right t))]))