...
(integer->charlist 10)
(integer->charlist 100)
(integer->charlist 128)
(char->integer #\d)
...
(intlist->string '(1 2 3 9 10))
(intlist->string '(1 2 10))
(char->integer #\newline)
...
(define (RSA-transform number key) ;;transform szyfruje!!!
  (expmod number (key-exponent key) (key-modulus key))) ;;(expmod number e n)
...
(define (solve-ax+by=1 a b)
  (define (// a b)
    (/ (- a (modulo a b)) b))
  (define (helper para)
    (let ((a (car para)) (b (cdr para)))
      (if (not (= b 0))
          (if (> a b)
              ((lambda (pom)
                 (cons (cdr pom) (- (car pom) (* (// a b) (cdr pom)))))
               (helper (cons b (modulo a b))))
              ((lambda (pom)
                 (cons (cdr pom) (- (car pom) (* (// b a) (cdr pom)))))
               (helper (cons a (modulo b a)))))          
          (cons 1 0))))
 (cdr (helper (cons a b))))
(solve-ax+by=1 (* 16 18) 17)
...
(define (RSA-unconvert-list intlist key)
  (let ((n (key-modulus key)))
    (define (unconvert l sum)
      (if (null? l)
          '()
          (let ((x (modulo (+ (RSA-transform (modulo (+ (car l) 0) n)
                                  key) sum) n)))
            (cons x (unconvert (cdr l) (car l))))))
    (unconvert intlist 0)))
  ;;; XXX: Write your code here
  ;;(let ((n (key-modulus key))
        ;;(define (unconvert u sum)
...
;;(reverse-rek (list 1 2 3 4 5 6))

(RSA-convert-list (list 1 2 3 4 5 6) (make-key 3233 17))
(RSA-unconvert-list (RSA-convert-list (list 1 2 3 4 5 6) (make-key 3233 17))
                    (make-key 3233 413))
(RSA-unconvert-list (RSA-convert-list (list 1 2 3 4 5 6) (make-key (* 17 19) 17))
                    (make-key (* 17 19) 17))

(RSA-unconvert-list (RSA-convert-list (list 0 0 0 0 0 0) (make-key 3233 17))
                    (make-key 3233 413))
;;(RSA-unconvert-list (reverse-rek (RSA-convert-list (list 1 2 3 4 5 6) (make-key 3233 17)))
                    ;;(make-key 3233 413))
;; sum jest po to zeby (1 2) -> (1 1) -> (1 2)
;; gdy sum==0 to tez dziala, z wyjatkiem duplikatow w szyfrogramie, co do ktorych istnienia warto by dac przyklad, bo sam go nie widze
;; wiec nierozumiem czemu bez sum by sie nieobylo
;; z pewnoscia sum sie by przydal jakby miec n+1 roznych znakow, ale to raczej nie o to chodzi
...
(define (make-signed-message message key-private) ;;Zwraca podpisana i niezaszyfrowana wiadomosc
...
(define (encrypt-and-sign message key-public key-private) ;; zwraca pare, ktorej pierwszym elementem jest zaszyfrowana wiadomosc kluczem publicznym, a drugim podpis (liczba)
...
(define (authenticate-and-decrypt encrypted-message key-private key-public)
  (if (= (signature encrypted-message) (car (string->intlist (RSA-decrypt (list (signature encrypted-message)) key-public)))) ;;sprawdza czy podpis == i tu nie jestem pewien czy powinno sie odszyfrowywac czy zaszyfrowywac podpis
      (RSA-decrypt message key-private)
      (error "Oszust!")))
(RSA-decrypt '(419 1746) (make-key 3233 413))
(message (encrypt-and-sign "Antoni" (make-key 3233 17) (make-key 3233 413)))
(signature (encrypt-and-sign "Antoni" (make-key 3233 17) (make-key 3233 413)))  ;;2766

;;(intlist->string (RSA-unconvert-list '(2399) (make-key 3233 413)))

(car (string->intlist (RSA-decrypt (list (signature (encrypt-and-sign "Antoni" (make-key 3233 17) (make-key 3233 413)))) (make-key 3233 17))));;2036 przy szyfrowaniu
(car (RSA-encrypt (intlist->string (list (signature (encrypt-and-sign "Antoni" (make-key 3233 17) (make-key 3233 413))))) (make-key 3233 17)));;67635255 przy deszyfrowaniu
(authenticate-and-decrypt (encrypt-and-sign "Antoni" (make-key 3233 17) (make-key 3233 413))
                          (make-key 3233 413) (make-key 3233 17))  ;;2766 != 2036 wiec wyrzuca blad, nieudalo mi sie ustalic w ktorym miejscu nastepuje przeoczenie i zla zamiana, a protokol wydaje sie byc zgodny z instrukcja
